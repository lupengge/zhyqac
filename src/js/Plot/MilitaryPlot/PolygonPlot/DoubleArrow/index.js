...
export default class DoubleArrow extends Polygon {
    ...
    constructor(viewer, geoFeature) {
        ...
        this.properties.plotType = PlotTypes.DOUBLE_ARROW; //标绘类型
        this.properties.plotName = "钳击"; //标绘名称 
    }
    ...
    generate() {
        ...
        var pnt3 = this.coordinates[0][2];
        var count = this.getPointCount();
        if (count == 3)
            this.tempPoint4 = this.getTempPoint4(pnt1, pnt2, pnt3);
        else
            this.tempPoint4 = this.coordinates[0][3];
        if (count == 3 || count == 4)
            this.connPoint = PlotUtils.mid(pnt1, pnt2);
        else
            this.connPoint = this.coordinates[0][4];
