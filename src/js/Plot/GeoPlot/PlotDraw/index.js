...
export default class PlotDraw {
    ...
    generateGeoFeature(plotCode, positions) {
        ...
        switch (this.plotType) {
            ...
            case PlotTypes.TEXT:
                geometryType = "Point";
                coordinates = this.getPointCoordinates(positions);
                break;
            case PlotTypes.POLYLINE:
            case PlotTypes.CIRCLE:
            case PlotTypes.RECTANGLE:
    ...
    setTipContent() {
        switch (this.plotType) {
            case PlotTypes.POLYLINE:
                ...
                break;
            case PlotTypes.POLYGON:
                this.plotDrawTip.setContent(["当前绘制类型：面，最少需要3个点。",
                    "已有" + this.positions.length + "个点，" + "按下鼠标左键确定第" + (this.positions.length + 1) + "个点",
                    this.positions.length < 3 ? "按下鼠标右键取消绘制" : "按下鼠标右键结束绘制"
                ])
                break;
            case PlotTypes.CIRCLE:
                this.plotDrawTip.setContent(["当前绘制类型：圆，需要2个点。",
                    "已有" + this.positions.length + "个点，" + "按下鼠标左键确定第" + (this.positions.length + 1) + "个点",
                    "按下鼠标右键取消绘制"
                ])
                break;
            case PlotTypes.RECTANGLE:
                this.plotDrawTip.setContent(["当前绘制类型：矩形，需要2个点。",
                    "已有" + this.positions.length + "个点，" + "按下鼠标左键确定第" + (this.positions.length + 1) + "个点",
                    "按下鼠标右键取消绘制"
                ])
                break;
