...
export default {
  /**@type {Array.<Entity>} 存放画出来的图形*/
  entities: [],

  //测量空间直线距离
  /******************************************* */
  measureLineSpace(viewer, handler) {
    window.measureing = true;
    let that = this;
    // 取消双击事件-追踪该位置
    ...
    handler.setInputAction(function (movement) {
      ...
      that.entities.push(floatingPoint)
  ...
  measureAreaSpace(viewer, handler) {
    ...
    function getArea(points) {
      ...
      if (res > 1000000) {
        return (res / 1000000.0).toFixed(3) + '平方公里'
      } else {
        return res.toFixed(3) + '平方米'
      }
    }
